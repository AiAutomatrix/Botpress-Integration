...
__name(error, "error");
function log(_2, title, ...messages) {
  if (!process.env.VERBOSE) {
    return;
  }
...
function isSchemaLike(schema) {
  ...
  if (JSON_SCHEMA_KEYWORDS.some((_2) => parent[_2] === schema)) {
    return false;
  }
  return true;
}
...
function declareNamedInterfaces(ast, options, rootASTName, processed = /* @__PURE__ */ new Set()) {
  ...
  switch (ast.type) {
    ...
    case "TUPLE":
    case "UNION":
      type = ast.params.map((_2) => declareNamedInterfaces(_2, options, rootASTName, processed)).filter(Boolean).join("\n");
...
function generateSetOperation(ast, options) {
  const members = ast.params.map((_2) => generateType(_2, options));
  const separator = ast.type === "UNION" ? "|" : "&";
  return members.length === 1 ? members[0] : "(" + members.join(" " + separator + " ") + ")";
}
__name(generateSetOperation, "generateSetOperation");
function generateInterface(ast, options) {
  return "{\n" + ast.params.filter((_2) => !_2.isPatternProperty && !_2.isUnreachableDefinition).map(
...
rules.set("Default additionalProperties", (schema, _2, options) => {
  ...
  switch (ast.type) {
    case "INTERFACE":
      return Object.assign(ast, {
        params: ast.params.map((_2) => Object.assign(_2, { ast: optimize(_2.ast, options, processed) }))
      });
    case "INTERSECTION":
    case "UNION":
      const optimizedAST = Object.assign(ast, {
        params: ast.params.map((_2) => optimize(_2, options, processed))
      });
      if (optimizedAST.params.some((_2) => _2.type === "ANY")) {
        log("cyan", "optimizer", "[A, B, C, Any] -> Any", optimizedAST);
        return T_ANY;
      }
      if (optimizedAST.params.some((_2) => _2.type === "UNKNOWN")) {
        log("cyan", "optimizer", "[A, B, C, Unknown] -> Unknown", optimizedAST);
        return T_UNKNOWN;
      }
      if (optimizedAST.params.every((_2) => {
        const a = generateType(omitStandaloneName(_2), options);
        const b = generateType(omitStandaloneName(optimizedAST.params[0]), options);
        return a === b;
      }) && optimizedAST.params.some((_2) => _2.standaloneName !== void 0)) {
        log("cyan", "optimizer", "[A (named), A] -> [A (named)]", optimizedAST);
        optimizedAST.params = optimizedAST.params.filter((_2) => _2.standaloneName !== void 0);
      }
      const params = (0, import_uniqBy.default)(optimizedAST.params, (_2) => generateType(_2, options));
      ...
      return Object.assign(optimizedAST, {
        params: optimizedAST.params.map((_2) => optimize(_2, options, processed))
      });
    default:
      return ast;
  }
...
function parseNonLiteral(schema, type, options, keyName, processed, usedNames) {
  ...
  const keyNameFromDefinition = (0, import_findKey.default)(definitions, (_2) => _2 === schema);
  switch (type) {
    case "ALL_OF":
      return {
        ...
        params: schema.allOf.map((_2) => parse(_2, options, void 0, processed, usedNames)),
        type: "INTERSECTION"
      };
    case "ANY":
    ...
    case "ANY_OF":
      return {
        ...
        keyName,
        standaloneName: standaloneName(schema, keyNameFromDefinition, usedNames),
        params: schema.anyOf.map((_2) => parse(_2, options, void 0, processed, usedNames)),
        type: "UNION"
      };
    case "BOOLEAN":
      return {
    ...
    case "NAMED_ENUM":
      return {
        ...
        params: schema.enum.map((_2, n) => ({
          ast: parseLiteral(_2, void 0),
    ...
    case "ONE_OF":
      return {
        ...
        keyName,
        standaloneName: standaloneName(schema, keyNameFromDefinition, usedNames),
        params: schema.oneOf.map((_2) => parse(_2, options, void 0, processed, usedNames)),
        type: "UNION"
      };
    case "REFERENCE":
    ...
    case "TYPED_ARRAY":
      if (Array.isArray(schema.items)) {
        ...
        const arrayType2 = {
          ...
          minItems: minItems2,
          standaloneName: standaloneName(schema, keyNameFromDefinition, usedNames),
          params: schema.items.map((_2) => parse(_2, options, void 0, processed, usedNames)),
          type: "TUPLE"
        };
    ...
    case "UNNAMED_ENUM":
      return {
        ...
        keyName,
        standaloneName: standaloneName(schema, keyNameFromDefinition, usedNames),
        params: schema.enum.map((_2) => parseLiteral(_2, void 0)),
        type: "UNION"
      };
    case "UNNAMED_SCHEMA":
...
function parseSuperTypes(schema, options, processed, usedNames) {
  ...
  return superTypes.map((_2) => parse(_2, options, void 0, processed, usedNames));
}
...
rules2.set("tsEnumNames must be an array of strings", (schema) => {
  if (schema.tsEnumNames && schema.tsEnumNames.some((_2) => typeof _2 !== "string")) {
    return false;
  }
});
...
var isPrimitive2 = /* @__PURE__ */ __name((type) => Primitives.includes(type), "isPrimitive");
...
function toTypescript(schema, options) {
  ...
  if ((_b = options.formatters) == null ? void 0 : _b.length) {
...
var _ZodType = class _ZodType {
  ...
  async toTypescriptAsync(opts) {
    let result = toTypescript(this, __spreadProps(__spreadValues({}, opts), { formatters: [] }));
    for (const formatter of (opts == null ? void 0 : opts.formatters) || []) {
...
((util4) => {
  ...
  util4.jsonStringifyReplacer = /* @__PURE__ */ __name((_2, value) => {
...
var FormElementRenderer = /* @__PURE__ */ __name((_a) => {
  ...
  if (fieldSchema.type === "array" && type === "array") {
    ...
    return /* @__PURE__ */ import_react5.default.createElement(Component3, __spreadProps(__spreadValues({ key: baseProps.scope }, props2), { isArrayChild: props2.isArrayChild }), Array.isArray(props2.data) ? props2.data.map((_2, index) => {
